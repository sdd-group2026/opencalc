...
class MyPreferences(context: Context) {
    ...
    companion object {
        ...
        private const val KEY_MEMORY_VALUE = "darkempire78.opencalculator.MEMORY_VALUE"
        private const val KEY_VARIABLES = "darkempire78.opencalculator.VARIABLES"
    }
    ...
    // Persisted as the enum constant *name* (a stable String), not its ordinal,
    // so ScientificModeTypes can be reordered without corrupting saved prefs.
    // MyPreferenceMigrator normalises any legacy boolean/ordinal value on read.
    var scientificMode: ScientificModeTypes =
        MyPreferenceMigrator.getScientificMode(preferences, KEY_SCIENTIFIC_MODE_ENABLED_BY_DEFAULT)
        set(value) = preferences.edit()
            .putString(KEY_SCIENTIFIC_MODE_ENABLED_BY_DEFAULT, value.storageKey).apply()
    ...
    // Calculator memory (MC/MR/M+/M−/MS). Stored as the BigDecimal's plain string,
    // null when memory is empty. A scalar value uses a plain String pref like
    // numberPrecision/historySize; Gson here is reserved for collections (history).
    var memoryValue = preferences.getString(KEY_MEMORY_VALUE, null)
        set(value) = preferences.edit().putString(KEY_MEMORY_VALUE, value).apply()


    fun getVariables(): MutableList<Variable> {
        val json = preferences.getString(KEY_VARIABLES, null) ?: return mutableListOf()
        return try {
            Gson().fromJson(json, Array<Variable>::class.java).toMutableList()
        } catch (e: Exception) {
            mutableListOf()
        }
    }

    fun saveVariables(variables: List<Variable>) {
        preferences.edit().putString(KEY_VARIABLES, Gson().toJson(variables)).apply()
    }
