...
import com.darkempire78.opencalculator.calculator.CalculationResult
import com.darkempire78.opencalculator.calculator.Calculator
import com.darkempire78.opencalculator.calculator.Memory
import com.darkempire78.opencalculator.calculator.MemoryResult
import com.darkempire78.opencalculator.calculator.Reciprocal
import com.darkempire78.opencalculator.calculator.ReciprocalResult
import com.darkempire78.opencalculator.calculator.VariableResolveResult
import com.darkempire78.opencalculator.calculator.VariableResolver
import com.darkempire78.opencalculator.calculator.StepEvaluationResult
import com.darkempire78.opencalculator.calculator.StepEvaluator
import com.darkempire78.opencalculator.calculator.SignToggle
...
import com.darkempire78.opencalculator.dialogs.DateCalculatorDialog
import com.darkempire78.opencalculator.dialogs.StepsDialog
import com.darkempire78.opencalculator.dialogs.UnitConverterDialog
import com.darkempire78.opencalculator.dialogs.VariablesDialog
...
class MainActivity : AppCompatActivity() {
    ...
    // Structured result of the most recent evaluation. equalsButton() relies on
    // this instead of the old shared global error flags.
    private var lastCalculationResult: CalculationResult =
        CalculationResult.Success(BigDecimal.ZERO)
    ...
    fun openUnitConverter(menuItem: MenuItem) {
        UnitConverterDialog(this, layoutInflater).open()
    }

    fun openDateCalculator(menuItem: MenuItem) {
        DateCalculatorDialog(this, layoutInflater).open()
    }

    fun openVariables(menuItem: MenuItem) {
        VariablesDialog(this, layoutInflater) { name -> updateDisplay(binding.input, name) }.open()
    }

    fun openSteps(menuItem: MenuItem) {
        val steps = when (val result = StepEvaluator.evaluate(normalizedExpressionForSteps())) {
            is StepEvaluationResult.Success -> result.steps.map { it.description }
            StepEvaluationResult.Unsupported -> null
        }
        StepsDialog(this, layoutInflater, steps).open()
    }

    /** Resolves variables and normalises the current input for the step tracer. */
    private fun normalizedExpressionForSteps(): String {
        val raw = binding.input.text.toString()
        val resolved = VariableResolver.resolve(raw, MyPreferences(this).getVariables())
        val expression = if (resolved is VariableResolveResult.Success) resolved.expression else raw
        return expression
            .replace(groupingSeparatorSymbol, "")
    ...
    private fun enableOrDisableScientistMode() {
        if (binding.scientistModeRow2.visibility != View.VISIBLE) {
            ...
            binding.memoryRow?.visibility = View.VISIBLE
            ...
            binding.memoryRow?.visibility = View.GONE
        ...
        lifecycleScope.launch(Dispatchers.Default) {
            ...
            if (calculation != "") {
                val resolved = VariableResolver.resolve(
                    calculation,
                    MyPreferences(this@MainActivity).getVariables()
                )
                if (resolved is VariableResolveResult.UnknownVariable) {
                    withContext(Dispatchers.Main) {
                        setErrorColor(true)
                ...
                val calculationTmp = Expression().getCleanExpression(
                    (resolved as VariableResolveResult.Success).expression,
                ...
                if (result is CalculationResult.Success) {
                    ...
                    if (result is CalculationResult.Error.Infinity) {
    ...
    fun eButton(view: View) {
        if (!isInvButtonClicked) {
            ...
            // EE: scientific-notation entry. The engine maps "E" to "*10^".
            updateDisplay(view, "E")
        }
    }
    ...
    fun toggleSignButton(view: View) {
        keyVibration(view)
        val result = SignToggle.toggle(
            binding.input.text.toString(),
            binding.input.selectionStart,
            decimalSeparatorSymbol,
            groupingSeparatorSymbol
        )
        binding.input.setText(result.text)
        binding.input.setSelection(result.cursorPosition)
    }

    fun memoryStore(view: View) {
        keyVibration(view)
        saveMemory(calculationResult)
    }
    ...
    fun memoryAdd(view: View) {
        keyVibration(view)
        saveMemory(Memory.add(readMemory(), calculationResult))
    }

    fun memorySubtract(view: View) {
        keyVibration(view)
        saveMemory(Memory.subtract(readMemory(), calculationResult))
    }

    fun memoryRecall(view: View) {
        when (val recalled = Memory.recall(readMemory())) {
            is MemoryResult.Value -> updateDisplay(view, formatMemoryValue(recalled.amount))
            MemoryResult.Empty -> keyVibration(view)
        }
    }

    /** Reads the persisted memory value, or null when memory is empty. */
    private fun readMemory(): BigDecimal? =
        MyPreferences(this).memoryValue?.toBigDecimalOrNull()

    /** Persists [value] (null clears memory) and refreshes the "M" indicator. */
    private fun saveMemory(value: BigDecimal?) {
        MyPreferences(this).memoryValue = value?.toPlainString()
        updateMemoryIndicator()
    }

    /** Shows the "M" indicator only when memory currently holds a value. */
    private fun updateMemoryIndicator() {
        binding.memoryIndicator?.isVisible = readMemory() != null
    }

    /** Formats a recalled memory value for insertion into the input field. */
    private fun formatMemoryValue(amount: BigDecimal): String =
        amount.toPlainString().replace(".", decimalSeparatorSymbol)

    fun reciprocalButton(view: View) {
        keyVibration(view)
        when (val result = Reciprocal.of(calculationResult)) {
            is ReciprocalResult.Success -> setInputToValue(result.value)
            ReciprocalResult.DivisionByZero ->
                binding.resultDisplay.text = getString(R.string.division_by_0)
        }
    }

    /** Replaces the input with a formatted [value] and moves the caret to the end. */
    private fun setInputToValue(value: BigDecimal) {
        val text = NumberFormatter.format(
            value.toPlainString(), decimalSeparatorSymbol, groupingSeparatorSymbol, numberingSystem
        )
        binding.input.setText(text)
        binding.input.setSelection(binding.input.text.length)
    }

    fun cubeRootButton(view: View) {
        updateDisplay(view, "∛")
    }

    fun nthRootButton(view: View) {
        // y-th root of x is x^(1/y); reuses the existing power/parenthesis engine.
    ...
    fun invButton(view: View) {
        ...
        if (!isInvButtonClicked) {
            ...
            binding.eButton?.setText(R.string.eeInv)
            binding.logarithmButton.setText(R.string.logarithmInv)
            binding.log2Button?.setText(R.string.logtwoInv)
            binding.hsinButton?.setText(R.string.hsinInv)
            binding.hcosButton?.setText(R.string.hcosInv)
            binding.htanButton?.setText(R.string.htanInv)
            if (MyPreferences(this).addModuloButton) {
            ...
            binding.eButton?.setText(R.string.e)
            binding.logarithmButton.setText(R.string.logarithm)
            binding.log2Button?.setText(R.string.logtwo)
            binding.hsinButton?.setText(R.string.hsin)
            binding.hcosButton?.setText(R.string.hcos)
            binding.htanButton?.setText(R.string.htan)
    ...
    fun equalsButton(view: View) {
        lifecycleScope.launch(Dispatchers.Default) {
            ...
            if (calculation != "") {
                ...
                // If result is a number and it is finite
                if (lastCalculationResult is CalculationResult.Success) {
                    ...
                    withContext(Dispatchers.Main) {
                        when (val result = lastCalculationResult) {
                            is CalculationResult.Error.SyntaxError -> {
                                setErrorColor(true)
                                binding.resultDisplay.text = getString(R.string.syntax_error)
                            }
                            is CalculationResult.Error.DomainError -> {
                                setErrorColor(true)
                                binding.resultDisplay.text = getString(R.string.domain_error)
                            }
                            is CalculationResult.Error.RequireRealNumber -> {
                                setErrorColor(true)
                                binding.resultDisplay.text = getString(R.string.require_real_number)
                            }
                            is CalculationResult.Error.DivisionByZero -> {
                                setErrorColor(true)
                                binding.resultDisplay.text = getString(R.string.division_by_0)
                            }
                            is CalculationResult.Error.Infinity -> {
                                if (result.value < BigDecimal.ZERO) binding.resultDisplay.text = "-" + getString(
                                    R.string.infinity
                                )
                                else binding.resultDisplay.text = getString(R.string.value_too_large)
    ...
    override fun onResume() {
        ...
        if (resources.configuration.orientation != Configuration.ORIENTATION_LANDSCAPE) {
            ...
            scientificModeType = MyPreferences(this).scientificMode
